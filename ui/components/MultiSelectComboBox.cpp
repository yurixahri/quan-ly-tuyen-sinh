...
void MultiSelectComboBox::stateChanged(int aState)
{
    ...
    for (int i = 1; i < count; ++i)
    {
        ...
        if (checkBox->isChecked())
        {
            ...
            selectedValues.append(values[checkBox]);
        }
    }
...
void MultiSelectComboBox::setSelectedValues(QList<QVariant> values){
    ResetSelection();
    selectedValues = values;

    for (auto &value : values){
        QMapIterator<QCheckBox*, QVariant> ptr(this->values);
        while (ptr.hasNext()) {
            ptr.next();
            if (value == ptr.value()){
                ptr.key()->setChecked(true);
                break;
            }
        }
    }

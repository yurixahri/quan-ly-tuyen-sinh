#ifndef MA_KVUT_DAO_H
#define MA_KVUT_DAO_H

#include "db/models/ma_kvut.h"
#include "utils/string.h"

inline bool addMaKvut(QString &ma, QString &ten, float &diem){
    ma_kvut_ptr adding;
    adding.reset(new ma_kvut());
    adding->ma = ma;
    adding->ten = ten;
    adding->diem = diem;
...
inline bool changeMaKvut(ma_kvut_ptr &item, QString &ma, QString &ten, float &diem){
    item->ma = ma;
    item->ten = ten;
    item->diem = diem;
...
inline std::optional<QList<ma_kvut_ptr>> getAllMaKvut(){
    QList<ma_kvut_ptr> list;
...
inline std::optional<ma_kvut_ptr> getMaKvutById(long &id){
    ma_kvut_ptr item;
    item.reset(new ma_kvut(id));
...
inline std::optional<ma_kvut_ptr> getMaKvutByName(QString &name){
    QList<ma_kvut_ptr> list;
    qx_query query;
...
inline bool deleteMaKvutById(long &id){
    ma_kvut_ptr item;
    item.reset(new ma_kvut(id));
